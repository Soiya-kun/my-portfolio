...
## 定性の説明と実績の分け方
各要件セクションでは、見出し・一行のリード文（`<p class="section-lead">`）・セクション全体の要約（200〜300字程度、起承転結でまとめる）を常に表示します。要約は本文の書き出しではなく、詳細と実績の内容を通して短くまとめたものにします。長い説明は「詳細を読む」パネルに、具体的な実績は「実績：<短い説明>」パネルに入れ、詳細 → 実績の順に置きます。項目によっては共感や姿勢を書くため、パネルの名前に「スキル」は使いません。短いセクションは詳細パネルを作らず、全文を表示します。パネルは次の形です。
...
実績パネルの summary は「実績：<短い説明>」の形にして、開く前に中身が分かるようにします。
