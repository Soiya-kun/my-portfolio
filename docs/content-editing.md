...
## 定性の説明と実績の分け方
各要件セクションでは、見出し・一行のリード文（`<p class="section-lead">`）・セクション全体の要約（200〜300字程度、起承転結でまとめる）を常に表示します。要約は本文の書き出しではなく、詳細と実績の内容を通して短くまとめたものにします。長い説明は「このスキルの詳細」パネルに、具体的な実績は「このスキルにまつわる実績」パネルに入れ、詳細 → 実績の順に置きます。短いセクションは詳細パネルを作らず、全文を表示します。パネルは次の形です。
