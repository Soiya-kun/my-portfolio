...
## 定性の説明と実績の分け方
各要件セクションでは、見出し・一行のリード文（`<p class="section-lead">`）・要点の段落を常に表示します。長い説明（目安として350字を超える部分）は「このスキルの詳細」パネルに、具体的な実績は「このスキルにまつわる実績」パネルに入れ、詳細 → 実績の順に置きます。短いセクションは詳細パネルを作らず、全文を表示します。パネルは次の形です。
